...
INT32U ctmp;
    ...
    {
        CtrlADC5 = (BIT7+BIT5+BIT1);  // CH2- 测VBAT通道, 此处为M通道专用控制器，采用直接赋值方式  //            
    }
    else
    {
        return;
...
float  Cal_temperature_data(void)
{
 ...
 uint32  temp;
    ...
    //从FLASH 的 0X420位置获取芯片内部的A/B/C/D/E 5个温度曲线系数，小端模式  //
    p=(uint8 code*)0x420;
    for(i=0;i<3;i++)
    {
    ...
    //从FLASH 的 0X480位置获取芯片内部的温度偏差值(芯片温度和环境温度的偏差)，10倍的系数，大端模式  //
    p=(uint8 code*)0x480;
    for(i=0;i<3;i++)
    {
