...
typedef struct  
{
    ...
    ST_U16_U08		 w_status;            //统一的测量类状态字，len=2 //
}MEASURE_VAR_DATA;


typedef struct  
{
    uint8  cnt_sec[3];        //状态判断秒计数器,0-功率反向1-旁路2-dimmer //
    uint8  cnt_dimmer;       // DIMMER 判断计数器 //
...
#define REV_ACPOWER_L      BIT0       //L 回路功率反向 //
#define REV_ACPOWER_N     BIT1       //N 回路功率反向 //
#define STATUS_DIMMER     BIT2       //DIMMER 计量模式 //
#define BYPASS_L                BIT3       //L 回路接地指示 //
#define BYPASS_N                BIT4       //L 回路接地指示 //
#define VBAT_LOW               BIT5       //电池电压低标识  //
